...
from data.russell1000_list import TICKERS as R1000

# ---------------------------------------------------------
# UNIVERSE (deduplicated)
# ---------------------------------------------------------
tickers = sorted(list(set(SP500 + NASDAQ100 + R1000)))

# ---------------------------------------------------------
# PAGE CONFIG
# ---------------------------------------------------------
st.set_page_config(page_title="Market Pattern Scanner", layout="wide")
st.title("📈 Dynamic Oscillation Model")
...
def passes_filters(df):
    ...
    # Volume filter
    if last_volume is None or last_volume < 1_000_000 or last_volume > 50_000_000:
        return False
